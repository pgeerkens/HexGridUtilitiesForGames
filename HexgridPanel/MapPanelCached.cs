  ...
  public sealed partial class CachedMapPanel : HexgridScrollable {
    ...
    private Bitmap PaintedCacheBuffer(string tag) {
      ...
      try {
        ...
        temp   = new Bitmap(width, height); temp.Tag = tag;
        temp.Paint(Point.Empty, CacheScale, DataContext.Model.PaintMap);
...
//    private const int CACHE_IS_DISPOSING = 2;
    ...
    protected override async void OnPaint(PaintEventArgs e) {
      ...
      if (BufferCache != null) {
        ...
        BufferMap    .Render(BufferUnits, location, mapScale, DataContext.Model.PaintUnits);
        BufferUnits  .Render(BufferBack,  location, mapScale, DataContext.Model.PaintHighlight);
        BufferShading.Render(BufferBack,  location, mapScale, DataContext.Model.PaintShading);
